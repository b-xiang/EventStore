	...
	public class AdminController : CommunicationController {
		...
		protected override void SubscribeCore(IHttpService service) {
			service.RegisterAction(
				...
				OnPostShutdown);
			service.RegisterAction(
				new ControllerAction("/admin/reloadconfig", HttpMethod.Post, Codec.NoCodecs, SupportedCodecs, new Operation(Operations.Node.Shutdown)),
				OnPostReloadConfig);
			service.RegisterAction(
		...
		private void OnPostReloadConfig(HttpEntityManager entity, UriTemplateMatch match) {
			if (entity.User != null &&
			    (entity.User.LegacyRoleCheck(SystemRoles.Admins) || entity.User.LegacyRoleCheck(SystemRoles.Operations))) {
				Log.Information("Reloading node configuration because reloadconfig command has been received.");
